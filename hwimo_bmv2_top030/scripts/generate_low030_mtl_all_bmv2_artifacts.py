#!/usr/bin/env python3
"""
Generate BMv2 artifacts for HWIMO low_030 MTL-all baseline.
"""
...
PROGRAMS = {
    "Hwimo_low030_mtl_all": [
        {
            "cluster": "low_030_mtl_all",
            "suffix": "1_2_3_4_5_6_7",
            "tasks": [1, 2, 3, 4, 5, 6, 7],
            "model_dir": "models/low_030_mtl_all",
            "model_file": "model_mtl_all_low_30.pkl",
        },
    ],
}
...
def load_cluster(root: Path, cfg: Dict[str, Any]) -> ClusterArtifacts:
    if "model_dir" in cfg:
        d = root / cfg["model_dir"]
    else:
        d = root / "models" / "top_030" / cfg["cluster"]

    model_file = cfg.get("model_file", "model_mtl_feature_selected_top_30.pkl")
    ...
    with warnings.catch_warnings():
        ...
        with open(d / model_file, "rb") as f:
            model = pickle.load(f)
...
def write_p4(root: Path, program: str, clusters: List[ClusterArtifacts]) -> Path:
    filename = {
        ...
        "Hwimo_low030_mtl_all": "hwimo_low030_mtl_all_bmv2.p4",
    }[program]
